# Implementation of *Debiasing Image-to-Image Translation Models*
**Inofficial** implementation of the debiasing framework of image-to-image models proposed in [Debiasing Image-to-Image Translation Models, Tanjim et al.](https://bmvc2022.mpi-inf.mpg.de/0182.pdf). This repository builds heavily upon the repository of the pSp-Paper [Encoding in Style: a StyleGAN Encoder for Image-to-Image Translation](https://github.com/eladrich/pixel2style2pixel) - License (MIT): https://github.com/eladrich/pixel2style2pixel/blob/master/LICENSE. 

The main differences between this repository and the original pSp repository can be summarized as follows:

1. **Data Augmentation:** Tanjim et al. propose to use shifting, shearing, scaling, and mirror flipping during training. I modified the `ImagesDataset` class to account for those augmentations. 

2. **Resampling:** To adress for class imbalances, Tanjim et al. leverage a stratified sampling scheme. Hence, I adjusted the `ImagesDataset` class such that it can load a labels `.csv` and I replaced the `DataLoader` by a `WeightedRandomSampler`. 

3. **Contrastive Learning:** I added the supervised constrastive learning loss using the [implementation by HobbitLong](https://github.com/HobbitLong/SupContrast/blob/master/losses.py). Prior to applying the contrastive loss, Tanjim et al. propose to standardize the latents and employ a small MLP that intends to map the standardized latents back to the space of unnormalized latents. Alternatively, you can also leave out the MLP and use the standardized latents for computing the contrastive loss but proceed with the unstandardized latents. I adjusted the `Coach` class and the `StyleGAN` class to implement the contrastive learning scheme. 

4. **Cross-Entropy Loss:** On top of the contrastive loss, Tanjim et al. employ a pretrained classifier and add a cross-entropy regularizer. This is implemented in the `Coach` class. Note, this code is tested on [FairFace](https://github.com/dchen236/FairFace) and implements only the FairFace classifier. Other classifiers---depending on your usecase---should be added to the `Coach` class. In contrast to the binary classifier employed in Tanjim et al., the FairFace classifier predicts out of seven ethnicities. 

5. **Curriculum Training:** Both new regularizers are accompanied by a regularization parameter that increases linearily. It starts at $0$ and is increased by $m$ every $k$ steps. 

## Setting up the Environment 
We provide all used packages in `requirements.txt`. Run 
```
pip -r install requirements.txt
```
and you should be ready to run the code.

## Training an Image Upsampling Algorithm  
Prior to training, be sure to adjust all relevant paths in `configs/paths_config.py`. For training the model on FairFace, you need to store the pretrained classifiers in `pretrained_models/fairface_classifier/`. You can download them [here](https://github.com/dchen236/FairFace). 

For the training commands, we refer to the original repository. Below, you can find all corresponding informations. 
Our changes can be adapted by passing the additional arguments arguments 
``` 
--dataset_type=fairface_super_resolution --train_labels_path=data/fairface_label_train.csv --use_augmentation --fairness_k=10000 --fairness_m=0.001 --use_mlp
``` 
to the training script. Note, when training the model on FairFace, we found that setting the flag `--use_mlp` leads to worse results. 

Inference can be performed exactly as in the pSp-repository. 

## Pretrained Models: 
To train the model, you need 
- the models necessary to run the code of the original pSp repository (see its `README.md` for more details);
- a pretrained StyleGAN2. For instance, for FairFace, we provide the following [StyleGAN2](https://drive.google.com/file/d/1PIQ7wCsKek0M80qgNNJkVmmGxMyAAcEE/view?usp=sharing); 
- a classifier for the cross-entropy loss. For instance, for FairFace, you can use the official [FairFace classifier](https://github.com/dchen236/FairFace)

All models should be stored in `pretrained_models/`

## TODOs: 

- [ ] Reproduce CelebA results in Tanjim et al. 
- [ ] Add Sketch-to-Sketch support. 
- [ ] Condense the `requirements.txt`.
